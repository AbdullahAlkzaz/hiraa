                ...
                <div class="card-body">
                    <div class="card-content">
                        <div class="row">
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <select class="form-control" id="target_audience" name="target_audience" required>
                                    ...
                                    <option value="children" {{ old('target_audience') == 'children' ? 'selected' : '' }}>
                                        Children</option>
                                    <option value="adults" {{ old('target_audience') == 'adults' ? 'selected' : '' }}>Adults
                                    </option>
                                    <option value="both" {{ old('target_audience') == 'both' ? 'selected' : '' }}>Both
                                    </option>
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="description" name="description" class="form-control editor" required>{{ old('description') }}</textarea>
                            @error('description')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div id="cardsContainer">
                    ...
                    <div class="card p-3 mb-3" id="card_0">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="card_body_0" name="cards[0][body]" class="form-control editor" required>{{ old('cards.0.body') }}</textarea>
                            @error('cards.0.body')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
        ...
        document.addEventListener('click', function(event) {
            if (event.target.classList.contains('deleteCardButton')) {
                ...
                tinymce.EditorManager.execCommand('mceRemoveEditor', true,
                    `card_body_${cardIndex}`); // تأكد من إزالة المحرر أولًا
                card.remove();
            }
        });
    </script>
